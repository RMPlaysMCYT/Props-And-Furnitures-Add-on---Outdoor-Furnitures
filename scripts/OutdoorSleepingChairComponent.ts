...
const SLEEPANIMATIONS = {
  sleep: "animation.player.sleeping"
}
...
//   try {
//     if (!entity) return;
//     const player = entity?.getComponent("rideable")?.getRiders();
//     if (!player) return;
//     player.playAnimation(SLEEPANIMATIONS.sleep, {stop });
//   } catch (error){}
// })

system.beforeEvents.startup.subscribe((InitEvent) => {
  InitEvent.blockComponentRegistry.registerCustomComponent("pafa_outdoor:ChairInteract2", {
    onPlayerInteract,
  });
});
